...
//					settingUpdate("clientSecret", sBLK)
			...
			if(ok4Main && getNestAuthToken()==sNULL) {
				...
				section(sectionTitleStr("Nest Authorization Page")) {
					...
					paragraph '<p style="color: red;">NOTICE: Please use the parent Nest account, Nest Family member accounts will not work correctly</p>', state: sCOMPLT
				}
			}
		}
	}
...
def devicesPage() {
	...
	if((String)settings.structures) {
		...
		section(sectionTitleStr("Select Devices:")) {
			...
			if(stats?.size() > 0) {
				input(name: "thermostats", title: imgTitle(getAppImg("i_th"), """<u>Nest Thermostats</u><small style="color: blue !important;"> (${stats?.size()} found)</small>""".toString()), type: sENUM, required: false, multiple: true, submitOnChange: true, options:stats)
			}
			state.thermostats = (List)settings.thermostats ? statState((List)settings.thermostats) : null
			if(coSmokes.size() > 0) {
				input(name: "protects", title: imgTitle(getAppImg("i_p"), """<u>Nest Protects</u><small style="color: blue !important;"> (${coSmokes?.size()} found)</small>""".toString()), type: sENUM, required: false, multiple: true, submitOnChange: true, options: coSmokes)
			}
			state.protects = (List)settings.protects ? coState((List)settings.protects) : null
			if(cams.size() > 0) {
				input(name: "cameras", title: imgTitle(getAppImg("i_c"), """<u>Nest Cameras</u><small style="color: blue !important;"> (${cams?.size()} found)</small>""".toString()), type: sENUM, required: false, multiple: true, submitOnChange: true, options: cams)
			}
...
def reviewSetupPage() {
	return dynamicPage(name: "reviewSetupPage", title: sBLK, install: true, uninstall: (Boolean)state.isInstalled) {
		section(sectionTitleStr("Device Setup Summary:")) {
			...
			paragraph '<p style="color: blue;">Tap <b>Done</b> to complete the install and create the devices selected</p>', state: sCOMPLT
		}
	}
}
...
*/
		...
		if(state.protects) {
			section (sectionTitleStr("Protect Device Names:")) {
				((Map<String,String>)state.protects)?.each { p ->
					found = true
		...
		if(state.cameras) {
			section (sectionTitleStr("Camera Device Names:")) {
				((Map<String,String>)state.cameras)?.each { c ->
					found = true
...
String getDevicesDesc(Boolean startNewLine=true) {
	...
	String str = sBLK
	str += pDev ? "${startNewLine ? "\n" : sBLK}<b>Physical Devices:</b>" : sBLK
	str += (List)settings.thermostats ? "\n • <i>[${((List)settings.thermostats)?.size()}] Thermostat${(((List)settings.thermostats)?.size() > 1) ? "s" : sBLK}</i>" : sBLK
	str += (List)settings.protects ? "\n • <i>[${((List)settings.protects)?.size()}] Protect${(((List)settings.protects)?.size() > 1) ? "s" : sBLK}</i>" : sBLK
	str += (List)settings.cameras ? "\n • <i>[${((List)settings.cameras)?.size()}] Camera${(((List)settings.cameras)?.size() > 1) ? "s" : sBLK}</i>" : sBLK

	str += vDev ? "${pDev ? "\n" : sBLK}\n<b>Virtual Devices:</b>" : sBLK
	str += state.vThermostats ? "\n • [${state.vThermostats?.size()}] Virtual Thermostat${(state.vThermostats?.size() > 1) ? "s" : sBLK}" : sBLK
	str += (Boolean)settings.presDevice ? "\n • <i>Presence Device</i>" : sBLK
	str += settings.weatherDevice ? "\n • <i>Weather Device Configured</i>" : sBLK
	str += (!(List)settings.thermostats && !(List)settings.protects && !(List)settings.cameras && !(Boolean)settings.presDevice) ? "\n • <i>No Devices Selected</i>" : sBLK
	return (str != sBLK) ? str : sNULL
}
...
def nestLoginPrefPage () {
	if(getNestAuthToken()==sNULL) {
		...
		return dynamicPage(name: "nestLoginPrefPage", title: "<h2>Nest Authorization Page</h2>", nextPage: getNestAuthToken() ? sBLK : "authPage", install: false) {
...
/*					String t1 = (String)dni.key
				...
				def nCameras = ((Map<String,String>)state.cameras).collect { dni ->
...
Boolean sendNestCmd(String cmdTypeId, String cmdType, String cmdObj, cmdObjVal, String childId) {
	...
	try {
		if(cmdTypeId) {
			...
			List<List> cmdQueue = (List<List>)atomicState."cmdQ${qnum}"
...
Integer getQueueToWork() {
	...
	cmdQueueList.eachWithIndex { val, idx ->
		List<List> cmdQueue = (List<List>)atomicState."cmdQ${idx}"
		if(cmdQueue?.size() > 0) {
...
void workQueue() {
	...
	List<List>t0=(List<List>)atomicState."cmdQ${qnum}"
	List<List> cmdQueue = t0 ?: []
	...
	try {
		if(cmdQueue?.size() > 0) {
			...
			if(!cmdIsProc()) {
				...
				cmdQueue = (List<List>)atomicState."cmdQ${qnum}"
...
def connectionStatus(String message, Boolean close = false) {
	String redirectHtml = close ? """<script>document.getElementsByTagName('html')[0].style.cursor = 'wait';setTimeout(function(){window.close()},2500);</script>""" : sBLK
	String html = """
		<!DOCTYPE html>
		<html>
		<head>
